...
class OCR:
    ...
    def crop(image):
        def click_and_crop(event, x, y, flags, param):
            ...
            # check to see if the left mouse button was released
                ...
                cv2.rectangle(image, OCR.refPt[0],
                              OCR.refPt[1], (0, 255, 0), 2)
                cv2.imshow("image", image)
    ...
    def pdf_convert(inpath, image_path):
        ...
        pil_images = pdf2image.convert_from_path(inpath, output_folder=OUTPUT_FOLDER, first_page=FIRST_PAGE, last_page=LAST_PAGE,
                                                 fmt=FORMAT, userpw=USERPWD, use_cropbox=USE_CROPBOX, strict=STRICT, poppler_path=POPPLER_PATH)
