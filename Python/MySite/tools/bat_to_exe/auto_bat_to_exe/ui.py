...
def start(open_mode):
    ...
    try:
        ...
        if open_mode == UIOpenMode.CHROME and chrome_available:
            eel.start('index.html', size=(650, 333), port=0)
        elif open_mode == UIOpenMode.USER_DEFAULT or (open_mode == UIOpenMode.CHROME and not chrome_available):
            eel.start('index.html', size=(650, 333), port=0, mode='user default')
        else:
            port = utils.get_port()
            print('Server starting at http://localhost:' + str(port) + '/index.html')
            eel.start('index.html', size=(650, 333), host='localhost', port=port, mode=None, close_callback=lambda x, y: None)
