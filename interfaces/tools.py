...
# convert text to excel pandas
def print_to_excel(output_file):
    df = pd.read_csv(io.StringIO(output), sep="\t", header=0)
...
class MainApp(App[str]):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Header()
        yield Select((str(value), str(key)) for key, value in options.items())
